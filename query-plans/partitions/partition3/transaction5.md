...
2020-12-21 16:02:21.125 CET [29856] CONTEXTO:  función PL/pgSQL get_seats_from_connection(integer) en la línea 5 en bucle FOR en torno a filas de un SELECT
...
2020-12-21 16:02:21.125 CET [29856] LOG:  duration: 345.629 ms  plan:
	...
	Aggregate  (cost=24.26..24.28 rows=1 width=4)
	  ...
	  ->  Function Scan on get_seats_from_connection  (cost=0.25..10.25 rows=500 width=4)
	        Filter: is_taken
2020-12-21 16:02:21.125 CET [29856] CONTEXTO:  sentencia SQL: «SELECT COUNT(seat)::real / (SELECT Count(*) FROM get_seats_from_connection($1))::real  FROM get_seats_from_connection($1)
					WHERE is_taken = TRUE»
	función PL/pgSQL calc_occupancy_rate(integer) en la línea 3 en asignación
```	
...
```sql
	...
	Result  (cost=442.59..442.60 rows=1 width=1)
	  InitPlan 1 (returns $0)
	    ->  Aggregate  (cost=442.58..442.59 rows=1 width=8)
	          ->  Append  (cost=0.00..442.56 rows=5 width=0)
	                ->  Seq Scan on passengers_schedule_0  (cost=0.00..89.32 rows=1 width=0)
	                      Filter: ((seat = $1) AND (connection = $2))
	                ->  Seq Scan on passengers_schedule_1  (cost=0.00..89.32 rows=1 width=0)
	                      Filter: ((seat = $1) AND (connection = $2))
	                ->  Seq Scan on passengers_schedule_2  (cost=0.00..89.32 rows=1 width=0)
	                      Filter: ((seat = $1) AND (connection = $2))
	                ->  Seq Scan on passengers_schedule_3  (cost=0.00..85.26 rows=1 width=0)
	                      Filter: ((seat = $1) AND (connection = $2))
	                ->  Seq Scan on passengers_schedule_4  (cost=0.00..89.32 rows=1 width=0)
	                      Filter: ((seat = $1) AND (connection = $2))
2020-12-21 16:02:21.138 CET [29856] CONTEXTO:  sentencia SQL: «SELECT (
			SELECT COUNT(*) FROM passengers_schedule WHERE seat = $1 AND connection = $2
		) > 0»
	...
	sentencia SQL: «SELECT COUNT(seat)::real / (SELECT Count(*) FROM get_seats_from_connection($1))::real  FROM get_seats_from_connection($1)
					WHERE is_taken = TRUE»
	función PL/pgSQL calc_occupancy_rate(integer) en la línea 3 en asignación
2020-12-21 16:02:21.138 CET [29856] LOG:  duration: 1.842 ms  plan:
	Query Text: SELECT (
						SELECT is_seat_occupied(s.seat_id, $1)
					)
