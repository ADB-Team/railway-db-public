...
Selecting cities where stations should be

```sql
2021-01-08 14:02:11.678 UTC [457] LOG:  duration: 0.038 ms  plan:
...
2021-01-08 14:02:11.678 UTC [457] CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."cities" x WHERE "city_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x"
```

Inserting stations and updating dependencies:

```sql
2021-01-08 14:02:11.678 UTC [457] LOG:  duration: 3.668 ms  plan:
	Query Text: /*
	...
	*
	* EXEC SCRIPT:
	* psql -v start_station="'START'" -v end_station="'END'" -v start_time="'DATE TIME'" -v train_id="'TRAIN'" [DB-NAME] < transactions/transaction3_cs3.sql
	*
	* TEST VALUES:
	psql -v train_id="'bywdfevctetzjdwvofemzctyerwshr'" [DB-NAME] < transactions/zedstore/transaction3_cs3.sql
	*/
...
2021-01-08 14:02:11.685 UTC [457] LOG:  duration: 0.026 ms  plan:
	Query Text: SELECT 1 FROM ONLY "public"."cities" x WHERE "city_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x
	LockRows  (cost=0.29..8.31 rows=1 width=10)
	  ->  Index Scan using cities_pkey on cities x  (cost=0.29..8.30 rows=1 width=10)
	        Index Cond: (city_id = 10544)
2021-01-08 14:02:11.685 UTC [457] CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."cities" x WHERE "city_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x"
2021-01-08 14:02:11.685 UTC [457] LOG:  duration: 2.668 ms  plan:
...
2021-01-08 14:02:11.711 UTC [457] LOG:  duration: 16.863 ms  plan:
	Query Text: INSERT INTO station_names_cs (SELECT station_id,station_name FROM stations);
	Insert on station_names_cs  (cost=0.00..353.07 rows=0 width=0)
	  ->  Seq Scan on stations  (cost=0.00..353.07 rows=19807 width=36)
2021-01-08 14:02:11.733 UTC [457] LOG:  duration: 0.016 ms  plan:
...
2021-01-08 14:02:11.733 UTC [457] CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."stations" x WHERE "station_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x"
2021-01-08 14:02:11.733 UTC [457] LOG:  duration: 0.010 ms  plan:
	Query Text: SELECT 1 FROM ONLY "public"."stations" x WHERE "station_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x
	LockRows  (cost=0.29..8.31 rows=1 width=10)
	  ->  Index Scan using stations_pkey on stations x  (cost=0.29..8.30 rows=1 width=10)
	        Index Cond: (station_id = 20001)
2021-01-08 14:02:11.733 UTC [457] CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."stations" x WHERE "station_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x"
2021-01-08 14:02:11.733 UTC [457] LOG:  duration: 0.004 ms  plan:
	Query Text: SELECT 1 FROM ONLY "public"."stations" x WHERE "station_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x
	LockRows  (cost=0.29..8.31 rows=1 width=10)
	  ->  Index Scan using stations_pkey on stations x  (cost=0.29..8.30 rows=1 width=10)
	        Index Cond: (station_id = 20001)
...
```sql
2021-01-08 14:02:11.733 UTC [457] LOG:  duration: 11.899 ms  plan:
	Query Text: INSERT INTO routes (start_station, end_station, distance, max_speed) VALUES
	((
			SELECT station_id FROM station_names_cs WHERE station_name = 'Görlitz HBF'
		), (
			SELECT station_id FROM station_names_cs WHERE station_name = 'Berlin Görlitzer Bhf'
		), 220, 300
	),
	((
			SELECT station_id FROM station_names_cs WHERE station_name = 'Berlin Görlitzer Bhf'
	...
	Insert on routes  (cost=517.50..517.53 rows=0 width=0)
	  InitPlan 1 (returns $0)
	  ...
	  InitPlan 3 (returns $2)
	    ->  Seq Scan on station_names_cs station_names_cs_2  (cost=0.00..129.38 rows=32 width=4)
	          Filter: ((station_name)::text = 'Berlin Görlitzer Bhf'::text)
	  InitPlan 4 (returns $3)
	    ->  Seq Scan on station_names_cs station_names_cs_3  (cost=0.00..129.38 rows=32 width=4)
	          Filter: ((station_name)::text = 'Görlitz HBF'::text)
...
## Generating ending times

```sql
2021-01-08 14:02:11.750 UTC [457] LOG:  duration: 0.027 ms  plan:
...
```sql
2021-01-08 14:02:11.750 UTC [457] CONTEXT:  SQL statement "SELECT $1 - (SELECT floor(random()*($1 / 4 - 0))+10)"
...
2021-01-08 14:02:11.750 UTC [457] LOG:  duration: 0.001 ms  plan:
...
2021-01-08 14:02:11.750 UTC [457] CONTEXT:  SQL statement "SELECT (SELECT result * interval '1 hour')"
	...
	PL/pgSQL function calc_end_time(timestamp without time zone,integer,integer,integer) line 3 at RETURN
2021-01-08 14:02:11.750 UTC [457] LOG:  duration: 0.742 ms  plan:
	Query Text: SELECT $1 + (
				SELECT calc_duration ((
					SELECT calc_avg_speed($2, $4)
				), $3))
...
2021-01-08 14:02:11.750 UTC [457] CONTEXT:  SQL statement "SELECT $1 + (
				SELECT calc_duration ((
					SELECT calc_avg_speed($2, $4)
				), $3))"
	PL/pgSQL function calc_end_time(timestamp without time zone,integer,integer,integer) line 3 at RETURN
2021-01-08 14:02:11.750 UTC [457] LOG:  duration: 0.003 ms  plan:
	Query Text: SELECT $1 - (SELECT floor(random()*($1 / 4 - 0))+10)
	Result  (cost=0.02..0.03 rows=1 width=8)
	  InitPlan 1 (returns $0)
	    ->  Result  (cost=0.00..0.02 rows=1 width=8)
...
Selecting routes and train for each connection:

```sql
2021-01-08 14:02:13.189 UTC [457] LOG:  duration: 0.011 ms  plan:
...
2021-01-08 14:02:13.189 UTC [457] CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."routes" x WHERE "route_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x"
2021-01-08 14:02:13.189 UTC [457] LOG:  duration: 0.018 ms  plan:
...
2021-01-08 14:02:13.189 UTC [457] CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."trains" x WHERE "train_id"::pg_catalog.text OPERATOR(pg_catalog.=) $1::pg_catalog.text FOR KEY SHARE OF x"
```

Finally create the connections:

```sql
2021-01-08 14:02:14.196 UTC [457] LOG:  duration: 2460.099 ms  plan:
	...
	Insert on schedule  (cost=1061.95..7830904.71 rows=0 width=0)
	  CTE selected_routes
	    ->  Seq Scan on routes  (cost=517.50..1033.76 rows=388 width=20)
