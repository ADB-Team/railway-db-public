...
CONTEXT:  SQL statement "SELECT (SELECT result * interval '1 hour')"
...
CONTEXT:  SQL statement "SELECT $1 - (SELECT floor(random()*($1 / 4 - 0))+10)"
...
CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."routes" x WHERE "route_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x"
...
CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."trains" x WHERE "train_id"::pg_catalog.text OPERATOR(pg_catalog.=) $1::pg_catalog.text FOR KEY SHARE OF x"
```
...
LOG:  statement: INSERT INTO stations (name, nr_platforms, city, date_created) VALUES (
		'Berlin Görlitzer Bhf', 6, (
			SELECT city_id FROM cities WHERE name LIKE '%Berlin%'
		), '1940-03-14'
	);
LOG:  duration: 0.199 ms  plan:
...
CONTEXT:  SQL statement "SELECT 1 FROM ONLY "public"."cities" x WHERE "city_id" OPERATOR(pg_catalog.=) $1 FOR KEY SHARE OF x"
LOG:  duration: 5.381 ms  plan:
...
Query Text: WITH selected_routes AS (
	...
	(
		...
		Insert on schedule  (cost=2164.80..7832017.56 rows=388000 width=56)
	  CTE selected_routes
	  ...
	  ->  Nested Loop  (cost=19.88..7829872.63 rows=388000 width=56)
	        ->  Nested Loop  (cost=0.00..4882.63 rows=388000 width=44)
	              ->  CTE Scan on starting_times  (cost=0.00..20.00 rows=1000 width=8)
...
## Explain analyze 
```
Insert on stations  (cost=373.00..373.02 rows=1 width=48) (actual time=4.214..4.214 rows=0 loops=1)
   InitPlan 1 (returns $0)
     ->  Seq Scan on cities  (cost=0.00..373.00 rows=100 width=4) (actual time=0.488..3.975 rows=1 loops=1)
           Filter: ((name)::text = 'Görlitz'::text)
           Rows Removed by Filter: 19999
   ->  Result  (cost=0.00..0.02 rows=1 width=48) (actual time=4.034..4.034 rows=1 loops=1)
 Planning time: 0.250 ms
 Trigger for constraint stations_city_fkey: time=0.352 calls=1
 Execution time: 4.636 ms
(9 filas)
```