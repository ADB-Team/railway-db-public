...
```sql
2021-01-07 13:40:47.284 UTC [863] LOG:  duration: 3.823 ms  plan:
	Query Text: SELECT (
	  ...
	  InitPlan 1 (returns $0)
	    ->  Aggregate  (cost=860.72..860.73 rows=1 width=8)
	          ->  Seq Scan on passengers_schedule  (cost=0.00..860.72 rows=1 width=0)
	                Filter: ((seat = 771852) AND (connection = 15506))
2021-01-07 13:40:47.284 UTC [863] CONTEXT:  SQL statement "SELECT (
			SELECT COUNT(*) FROM passengers_schedule WHERE seat = $1 AND connection = $2
		) > 0"
...
2021-01-07 13:40:47.284 UTC [863] LOG:  duration: 4.150 ms  plan:
	Query Text: SELECT (
						SELECT is_seat_occupied(s.seat_id, $1)
					)
...
```sql
2021-01-07 13:40:47.583 UTC [863] LOG:  duration: 105.085 ms  plan:
	Query Text: (
					SELECT seat_id
					FROM seats
	              ...
	              ->  Hash  (cost=16.61..16.61 rows=1 width=64)
	                    ->  Nested Loop  (cost=0.57..16.61 rows=1 width=64)
	                          ->  Index Scan using schedule_pkey on schedule  (cost=0.29..8.30 rows=1 width=32)
	                                Index Cond: (connection_id = 15506)
	                          ->  Index Only Scan using trains_pkey on trains  (cost=0.29..8.30 rows=1 width=32)
	                                Index Cond: (train_id = (schedule.train)::text)
2021-01-07 13:40:47.583 UTC [863] CONTEXT:  PL/pgSQL function get_seats_from_connection(integer) line 5 at FOR over SELECT rows
	SQL statement "SELECT COUNT(seat)::real / (SELECT Count(*) FROM get_seats_from_connection($1))::real  FROM get_seats_from_connection($1)
					WHERE is_taken = TRUE"
	PL/pgSQL function calc_occupancy_rate(integer) line 3 at assignment
2021-01-07 13:40:47.583 UTC [863] LOG:  duration: 393.184 ms  plan:
	...
	Aggregate  (cost=24.26..24.28 rows=1 width=4)
	  ...
	  ->  Function Scan on get_seats_from_connection  (cost=0.25..10.25 rows=500 width=4)
	        Filter: is_taken
2021-01-07 13:40:47.583 UTC [863] CONTEXT:  SQL statement "SELECT COUNT(seat)::real / (SELECT Count(*) FROM get_seats_from_connection($1))::real  FROM get_seats_from_connection($1)
					WHERE is_taken = TRUE"
	PL/pgSQL function calc_occupancy_rate(integer) line 3 at assignment
```
...
```sql
2021-01-07 13:40:47.586 UTC [863] LOG:  duration: 412.913 ms  plan:
	Query Text: /*
	...
	*/
	...
	Sort  (cost=11786.44..11904.06 rows=47045 width=76)
	  Sort Key: ((to_char(('100'::double precision * cor.rate), '999D99%'::text))::character varying) DESC
	  CTE connections
	    ->  Hash Join  (cost=581.31..1030.04 rows=97 width=76)
	          ...
	          ->  Hash  (cost=178.12..178.12 rows=48 width=20)
	                ->  Seq Scan on routes  (cost=0.00..178.12 rows=48 width=20)
	                      Filter: (end_station = $0)
	  ->  Hash Join  (cost=545.93..5012.30 rows=47045 width=76)
	        Hash Cond: (cor.connection = connections.connection_id)
	        ->  Nested Loop  (cost=0.25..2427.19 rows=97000 width=36)
	              ->  CTE Scan on connections connections_1  (cost=0.00..1.94 rows=97 width=4)
	              ->  Function Scan on calc_occupancy_rate cor  (cost=0.25..10.25 rows=1000 width=8)
	        ->  Hash  (cost=544.47..544.47 rows=97 width=48)
	              ->  Nested Loop  (cost=3.73..544.47 rows=97 width=48)
	                    ->  Nested Loop  (cost=3.44..506.40 rows=97 width=20)
	                          ->  Hash Join  (cost=3.15..474.81 rows=97 width=20)
	                                Hash Cond: (sch.connection_id = connections.connection_id)
	                                ->  Seq Scan on schedule sch  (cost=0.00..397.41 rows=19541 width=16)
	                                ->  Hash  (cost=1.94..1.94 rows=97 width=4)
	                                      ->  CTE Scan on connections  (cost=0.00..1.94 rows=97 width=4)
	                          ->  Index Scan using routes_pkey1 on routes r  (cost=0.29..0.33 rows=1 width=8)
	                                Index Cond: (route_id = sch.route)
	                    ->  Index Scan using stations_pkey on stations s  (cost=0.29..0.39 rows=1 width=36)
