...
```sql
2021-01-08 14:42:45.677 UTC [1091] LOG:  duration: 3.818 ms  plan:
	Query Text: SELECT (
...
2021-01-08 14:42:45.677 UTC [1091] CONTEXT:  SQL statement "SELECT (
			SELECT COUNT(*) FROM passengers_schedule WHERE seat = $1 AND connection = $2
		) > 0"
...
2021-01-08 14:42:45.677 UTC [1091] LOG:  duration: 4.137 ms  plan:
	Query Text: SELECT (
						SELECT is_seat_occupied(s.seat_id, $1)
					)
...
```sql
2021-01-08 14:42:51.567 UTC [1091] LOG:  duration: 91.604 ms  plan:
	Query Text: (
					SELECT seat_id
					FROM seats
...
2021-01-08 14:42:51.567 UTC [1091] CONTEXT:  PL/pgSQL function get_seats_from_connection(integer) line 5 at FOR over SELECT rows
	SQL statement "SELECT COUNT(seat)::real / (SELECT Count(*) FROM get_seats_from_connection($1))::real  FROM get_seats_from_connection($1)
					WHERE is_taken = TRUE"
	PL/pgSQL function calc_occupancy_rate(integer) line 3 at assignment
2021-01-08 14:42:51.567 UTC [1091] LOG:  duration: 448.041 ms  plan:
	...
	Aggregate  (cost=24.26..24.28 rows=1 width=4)
	  ...
	  ->  Function Scan on get_seats_from_connection  (cost=0.25..10.25 rows=500 width=4)
	        Filter: is_taken
2021-01-08 14:42:51.567 UTC [1091] CONTEXT:  SQL statement "SELECT COUNT(seat)::real / (SELECT Count(*) FROM get_seats_from_connection($1))::real  FROM get_seats_from_connection($1)
					WHERE is_taken = TRUE"
	PL/pgSQL function calc_occupancy_rate(integer) line 3 at assignment
2021-01-08 14:42:51.567 UTC [1091] LOG:  duration: 5999.966 ms  plan:
	Query Text: /*
